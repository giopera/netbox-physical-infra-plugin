# Testing Guide for Netbox Physical Infra Plugin

This guide explains how to write and run tests for Netbox Physical Infra Plugin.

## Test Structure

Tests are organized in the `netbox_physical_infra_plugin/tests/` directory:

```
netbox_physical_infra_plugin/
├── tests/
│   ├── __init__.py
...
```python
from netbox_physical_infra_plugin.testing import PluginTestCase
...
```python
from netbox_physical_infra_plugin.testing import PluginModelTestCase
...
```python
from netbox_physical_infra_plugin.testing import PluginAPITestCase

class MyAPITestCase(PluginAPITestCase):
    def setUp(self):
        super().setUp()
        self.list_url_name = 'plugins-api:netbox_physical_infra_plugin-api:mymodel-list'
        self.detail_url_name = 'plugins-api:netbox_physical_infra_plugin-api:mymodel-detail'
...
```python
from netbox_physical_infra_plugin.testing import PluginViewTestCase

class MyViewTestCase(PluginViewTestCase):
    def setUp(self):
        super().setUp()
        self.base_url = 'plugins:netbox_physical_infra_plugin:mymodel'
...
```python
from netbox_physical_infra_plugin.testing import PluginGraphQLTestCase
...
The `netbox_physical_infra_plugin.testing.utils` module provides helpful utilities:

```python
from netbox_physical_infra_plugin.testing.utils import (
...
user = create_test_user(permissions=['netbox_physical_infra_plugin.view_physicalinfra'])
...
python manage.py test netbox_physical_infra_plugin.tests

# Run specific test file
python manage.py test netbox_physical_infra_plugin.tests.test_models

# Run specific test class
python manage.py test netbox_physical_infra_plugin.tests.test_models.PhysicalinfraTestCase

# Run specific test method
python manage.py test netbox_physical_infra_plugin.tests.test_models.PhysicalinfraTestCase.test_create

# Run with verbose output
python manage.py test netbox_physical_infra_plugin.tests -v 2

# Run in parallel (faster)
python manage.py test netbox_physical_infra_plugin.tests --parallel

# Keep database between runs (faster during development)
python manage.py test netbox_physical_infra_plugin.tests --keepdb
```
...
# Run tests
docker-compose exec netbox python manage.py test netbox_physical_infra_plugin.tests

# Run specific tests
docker-compose exec netbox python manage.py test netbox_physical_infra_plugin.tests.test_api
...
def test_form_validation(self):
    ...
    url = reverse('netbox_physical_infra_plugin:mymodel_add')
...
**"No module named 'netbox_physical_infra_plugin'"**
